...
async function insertOsprey(filePath, triggeredBy = 'cron') {
  ...
  // A partial/empty report must NOT become the 'latest snapshot' — the deal
  // sync reads latest-snapshot-only and a tiny scrape blinds it (Sep 2026:
  // days of 1-row downloads went unnoticed). Fail loudly; the loop retries.
  const MIN_ROWS = Number(process.env.OSPREY_MIN_ROWS || 500);
  if (records.length < MIN_ROWS) throw new Error(`Osprey CSV too small (${records.length} rows < ${MIN_ROWS}) — refusing to snapshot a partial report`);
  ...
  // ── Weekly pipeline snapshot (append-only) ──────────────────────────────
  // Freezes what each est-week's pipeline looked like at capture time —
  // drop/quote/live counts and postage — so arrival ("backfill") and
  // quote-conversion curves can be computed from real history instead of
  // approximations (osprey_mail_drops itself is a rolling window). One row per
  // (capture_date, est_week); later scrapes the same day overwrite, so each
  // row reflects end-of-day state. Failures never break the Osprey sync.
  try {
    const wkStart = (d) => { const x = new Date(d + 'T00:00:00Z'); x.setUTCDate(x.getUTCDate() - x.getUTCDay()); return x.toISOString().slice(0, 10); };
    const todayWk = wkStart(captureDate);
    const weekAgg = {};
    for (const r of rows) {
      if (!r.drop_est_date) continue;
      if (/CANCEL|DISCARD/i.test(String(r.drop_status || ''))) continue;
      const w = wkStart(r.drop_est_date);
      const weeksOut = Math.round((new Date(w) - new Date(todayWk)) / (7 * 86400000));
      if (weeksOut < -4 || weeksOut > 16) continue;
      const a = weekAgg[w] || (weekAgg[w] = { weeks_out: weeksOut, drop_count: 0, quote_count: 0, live_count: 0, scheduled_postage: 0, live_postage: 0 });
      a.drop_count++;
      if (String(r.order_status || '').trim().toUpperCase() === 'QUOTE') a.quote_count++;
      if (r.is_live_status) { a.live_count++; a.live_postage += r.postage_amount || 0; }
      a.scheduled_postage += r.postage_amount || 0;
    }
    const snapRows = Object.entries(weekAgg).map(([w, a]) => ({ capture_date: captureDate, est_week: w, ...a }));
    if (snapRows.length) {
      const { error: wpsErr } = await supabase.from('weekly_pipeline_snapshots').upsert(snapRows, { onConflict: 'capture_date,est_week' });
      if (wpsErr) console.error('weekly_pipeline_snapshots upsert error:', wpsErr.message);
      else console.log(`  Captured weekly pipeline snapshot (${snapRows.length} weeks)`);
    }
  } catch (e) {
  ...
  // ── CRM live-sync hook ──────────────────────────────────────────────────
  // Fires only when live_sync_enabled is ON in crm_settings. Failures here
  // never break the Osprey sync — we catch + log so the cron task always
  // reports success based on the Osprey upserts above.
  try {
    const { syncChanged } = require('../dashboard/lib/crmSync');
    const crmRes = await syncChanged(supabase, { triggeredBy: triggeredBy || 'cron' });
    if (crmRes?.ok && !crmRes.skipped && crmRes.stats) {
      const s = crmRes.stats;
      console.log(`  CRM sync — ${s.created} created · ${s.updated} updated · ${s.skipped} skipped · ${s.conflict} conflicts · ${s.error} errors`);
    } else if (crmRes?.skipped) {
      console.log(`  CRM sync skipped (${crmRes.reason})`);
    }
  } catch (crmErr) {
    console.error('CRM sync failed (Osprey sync OK):', crmErr.message);
    // best-effort event log so the user sees the failure in /crm
    try {
      await supabase.from('crm_events').insert({
        event_type: 'sync_failed', status: 'error',
        title: 'CRM live sync threw an unhandled error',
        body: crmErr.message,
        created_by: 'cron',
      });
    } catch {}
