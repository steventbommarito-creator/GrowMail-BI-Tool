...
// Extract Osprey mail_drop_id from EPS Detail string.
// Typical shape: "DMM_4_13_26_STND_LTR_D_271515" — trailing underscore + digits is the drop id.
// Also tolerates a bare numeric detail value.
...
// Only GrowMail's EPS account belongs in usps_transactions. The BCG login can
// see other EPS accounts (a new one appeared 2026-08; six strays polluted the
// first April scrape) and the EPS report returns ALL of them — filter here so
// portal-side changes can never leak foreign accounts in again.
const EPS_ACCOUNT = process.env.USPS_EPS_ACCOUNT || '1000081569';
...
async function insertUSPS(filePath) {
  ...
  if (allRecords.length > 0) {
    console.log('  Columns detected:', Object.keys(allRecords[0]).join(', '));
  }

  const records = allRecords.filter((r) => String(r['EPS Account Number'] || '').trim() === EPS_ACCOUNT);
  if (records.length !== allRecords.length) {
    const dropped = {};
    for (const r of allRecords) {
      const a = String(r['EPS Account Number'] || '').trim();
      if (a !== EPS_ACCOUNT) dropped[a || '(blank)'] = (dropped[a || '(blank)'] || 0) + 1;
    }
    console.log(`  Dropped ${allRecords.length - records.length} rows from other EPS accounts:`, JSON.stringify(dropped));
  }
  ...
  const rows = records.map((r) => {
    ...
    // Run the parser on every row, regardless of is_dmm. Detail strings on
    // non-DMM rows like "PURCHASE 402278_GM_271239" still encode a drop id;
    // the parser is conservative (only matches a trailing _NNNN or bare
    // digits) so deposits and fee rows just return null and stay unmatched.
    const ospreyMailDropId = parseDropId(detail);
    ...
    return {
      ...
      job_id: detail || null,                 // "DMM_4_13_26_STND_LTR_D_271515", etc.
      osprey_mail_drop_id: ospreyMailDropId,  // parsed from Detail — used to suppress double-deduction
