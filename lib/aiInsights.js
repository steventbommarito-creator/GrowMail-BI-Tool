...
async function aiInsights() {
  ...
  const { data: committed } = await supabase
    ...
    .select('postage_amount, mail_drop_amount, fulfillment_path, product_category')
  ...
  const { data: upcoming } = await supabase
    .from('osprey_mail_drops')
    .select('drop_est_date, postage_amount, product_category, fulfillment_path')
    .gte('drop_est_date', new Date().toISOString().split('T')[0])
  ...
  const { data: recent } = await supabase
    .from('osprey_mail_drops')
    .select('fulfillment_path, product_category, postage_amount, mail_drop_amount')
    .gte('captured_at', since30);
  ...
  for (const r of recent || []) {
    ...
    byCategory[r.product_category] = (byCategory[r.product_category] || 0) + (r.postage_amount || 0);
  }
  ...
  for (const r of recent || []) {
    ...
    quotedByPath[r.fulfillment_path] = (quotedByPath[r.fulfillment_path] || 0) + (r.postage_amount || 0);
  }
  ...
  for (const r of upcoming || []) {
    ...
    weeklyForecast[label].postage += r.postage_amount || 0;
