...
async function aiInsights() {
  ...
  // Mirrors dashboard/lib/postage.js — kept inline here because this file is
  // CJS while the dashboard helper is ESM. Source-of-truth priority:
  //   1. mail_method === "LDP" → handled by LDP, exclude entirely ($0)
  //   2. actual_postage > 0    → real cost from Osprey production
  //   3. postage_amount > 0    → estimated postage from the report
  //   4. LDP Postcard fallback → quantity × LDP_POSTCARD_RATE
  const LDP_POSTCARD_RATE = 0.244;
  const eff = (r) => {
    if (!r) return 0;
    if ((r.mail_method || '').toUpperCase() === 'LDP') return 0;
    if (r.actual_postage && r.actual_postage > 0) return r.actual_postage;
    if (r.postage_amount  && r.postage_amount  > 0) return r.postage_amount;
    if ((r.product_category || '').toLowerCase().includes('ldp postcard')) {
      return (r.mail_drop_quantity || 0) * LDP_POSTCARD_RATE;
    }
    return 0;
  };
  ...
  const { data: committed } = await supabase
    ...
    .select('postage_amount, actual_postage, mail_method, mail_drop_amount, mail_drop_quantity, fulfillment_path, product_category')
  ...
  const { data: upcoming } = await supabase
    .from('osprey_mail_drops')
    .select('drop_est_date, postage_amount, actual_postage, mail_method, mail_drop_quantity, product_category, fulfillment_path')
    .gte('drop_est_date', new Date().toISOString().split('T')[0])
  ...
  const { data: recent } = await supabase
    .from('osprey_mail_drops')
    .select('fulfillment_path, product_category, postage_amount, actual_postage, mail_method, mail_drop_quantity, mail_drop_amount')
    .gte('captured_at', since30);
